// Original demp implmentation of hole_fixer from GitHub User Erkaman https://github.com/Erkaman/hole_fixer
// Modified to work with our content retrieval program
...
namespace MeshRepairer {
	...
	void repairMesh(MatrixXd V, MatrixXi F, MatrixXd& finalV, MatrixXi& finalF)
	{
		...
		igl::boundary_loop(F, originalLoop);
		...
		do {
			...
			// now, we fuse the patch and the original mesh together.
			{
				...
				for (int itri = 0; itri < F.rows(); ++itri) {

					int triIndices[3];
					for (int iv = 0; iv < 3; ++iv) {

						int triIndex = F(itri, iv);
						...
						if (originalToFusedMap.count(triIndex) == 0) {
							...
							for (int jj = 0; jj < patchV.rows(); ++jj) {
								...
								VectorXd v(3); v << V(triIndex, 0), V(triIndex, 1), V(triIndex, 2);
							...
							if (foundMatch != -1) {
								...
								fusedV.push_back({ V(triIndex, 0), V(triIndex, 1), V(triIndex, 2) });
